...
public:
	...
	//the device is a handle to talk to the device :) where
	//device means the gpu. So we use it for selecting a particular
	//gpu, do we want dx11 or dx12, making resources, etc.
	ID3D11Device& GetDevice() {
		assert(GetDeviceReady());
		return *mpd3dDevice;
	}
	//controls the settings on that device, so render states, submitting
	//draw calls, the context in which the resource on the device are used
